...
function isGeneratedAgentMemoryContent(content: string): boolean {
  return content.includes("# Repository Memory") &&
    content.includes(GENERATED_MEMORY_MARKER);
}
...
async function headTracksPath(repoPath: string, filename: string): Promise<boolean> {
  const result = await runGit(repoPath, ["cat-file", "-e", `HEAD:${filename}`]);
  return result.exitCode === 0;
}

async function readStagedFile(repoPath: string, filename: string): Promise<string | undefined> {
  const result = await runGit(repoPath, ["show", `:${filename}`]);
...
export async function removeGeneratedAgentMemoryFilesFromGit(
  worktreePath: string,
): Promise<void> {
  for (const filename of MEMORY_FILENAMES) {
    if (await headTracksPath(worktreePath, filename)) {
      continue;
    }

    const filePath = join(worktreePath, filename);
    const worktreeContent = (await pathExists(filePath))
      ? await readFile(filePath, "utf8")
      : undefined;
    const stagedContent = await readStagedFile(worktreePath, filename);
    const generated =
      (worktreeContent !== undefined && isGeneratedAgentMemoryContent(worktreeContent)) ||
      (stagedContent !== undefined && isGeneratedAgentMemoryContent(stagedContent));
    if (!generated) {
      continue;
    }

    await runGit(worktreePath, ["rm", "--cached", "--ignore-unmatch", "--", filename]);
    try {
